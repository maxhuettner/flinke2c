...
public class DataStream<T> {
    ...
    public SingleOutputStreamOperator<T> serverless(String operatorName) throws IOException {
		return transform(operatorName, getType(), new StreamCEPlessOperator<>(operatorName));
	}

	public SingleOutputStreamOperator<T> kMeans() throws IOException {
		return transform("k-means", getType(), new StreamKMeansOperator<>());
	}

	public SingleOutputStreamOperator<T> benchmark(int eventRate) throws IOException {
		return transform("Benchmark", getType(), new StreamBenchmarkOperator<>(eventRate));
	}

	public SingleOutputStreamOperator<T> forward() throws IOException {
    ...
    // TODO(MH): Check if problem with standard job
    // /**
