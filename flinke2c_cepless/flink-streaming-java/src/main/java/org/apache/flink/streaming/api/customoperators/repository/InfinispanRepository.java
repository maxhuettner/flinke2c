...
public class InfinispanRepository implements EventRepository, Serializable {
			...
			public void run() {
				...
				if (addr != null) {
					if (cacheManager.getCache(addr) == null) {
						cacheManager.administration().getOrCreateCache(addr, DefaultTemplate.DIST_SYNC);
					}
					if (channelIn == null) {
	...
	public void listen(String addr) {
		...
		if (this.cacheManager.getCache(addr) == null) {
			...
			this.cacheManager.administration().getOrCreateCache(addr, DefaultTemplate.DIST_SYNC);
		}
