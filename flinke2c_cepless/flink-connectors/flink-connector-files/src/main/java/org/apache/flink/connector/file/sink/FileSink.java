...
public class FileSink<IN>
    ...
    private BucketWriter<?, String> createBucketWriterUnchecked() {
        try {
            return bucketsBuilder.createBucketWriter();
        } catch (IOException e) {
            throw new FlinkRuntimeException("Could not create bucket writer.", e);
        }
    }

    private <T> SingleOutputStreamOperator<T> forwardUnchecked(DataStream<T> stream) {
        try {
            return stream.forward();
        } catch (IOException e) {
            throw new FlinkRuntimeException("Could not set forward partitioner.", e);
        }
    }
    ...
    public DataStream<CommittableMessage<FileSinkCommittable>> addPreCommitTopology(
        ...
        SerializableSupplierWithException<
                ...
                committableSerializerSupplier = this::getCommittableSerializer;
        SerializableSupplierWithException<BucketWriter<?, String>, IOException>
                bucketWriterSupplier = this::createBucketWriterUnchecked;
        ...
        if (strategy == null) {
            ...
            SingleOutputStreamOperator<
                    ...
                    coordinatorOp =
                            forwardUnchecked(committableStream)
                                    .transform(
            ...
            SingleOutputStreamOperator<CommittableMessage<FileSinkCommittable>> operator =
                    forwardUnchecked(coordinatorOp)
                            .transform(
                ...
                committableStream
                        ...
                        .transform(
                                ...
                                new CompactCoordinatorFactory(
                                        strategy, committableSerializerSupplier))
                        .setParallelism(1)
