...
func startOperator(cli *client.Client, imageId string, operatorIdentifier string, addrIn string, addrOut string) (container.ContainerCreateCreatedBody, error) {
	...
	if cpuLimitStr := os.Getenv("CPU_LIMIT"); cpuLimitStr != "" {
		cpuLimit, err := strconv.ParseFloat(cpuLimitStr, 64)
		if err != nil || cpuLimit <= 0 {
			log.Printf("Invalid CPU_LIMIT '%s'; ignoring", cpuLimitStr)
		} else {
			const cpuPeriod int64 = 100000
			hostConfig.Resources.CPUPeriod = cpuPeriod
			hostConfig.Resources.CPUQuota = int64(cpuLimit * float64(cpuPeriod))
		}
	}
...
func DeployNewOperator(operatorName string, operatorRequestIdentifier string, addrIn string, addrOut string) error {
	...
	defer out.Close()
	err = helper.DetectErrorMessage(out)
	if err != nil {
		panic(err)
	}
