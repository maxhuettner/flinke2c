...
public class CommonTestUtils {
	...
	private static Class<?> defineClass(
		...
		try {
			Method defineClass = sun.misc.Unsafe.class.getMethod(
					"defineClass",
					String.class,
					byte[].class,
					int.class,
			...
			return defineClassWithoutUnsafe(className, bytecode, targetClassLoader, protectionDomain);
		} catch (InvocationTargetException e) {
			rethrowInvocationTargetException(e);
			throw new AssertionError("Unreachable code");
		}
	}

	private static Class<?> defineClassWithoutUnsafe(
			String className,
			byte[] bytecode,
			ClassLoader targetClassLoader,
			ProtectionDomain protectionDomain) throws Exception {
		try {
			...
			try {
				lookup = (MethodHandles.Lookup) privateLookupIn.invoke(null, ClassLoader.class, MethodHandles.lookup());
			} catch (InvocationTargetException e) {
				rethrowInvocationTargetException(e);
				throw new AssertionError("Unreachable code");
			}
			MethodType signature = MethodType.methodType(
					...
					int.class,
					ProtectionDomain.class);
			MethodHandle defineClassHandle = lookup.findSpecial(ClassLoader.class, "defineClass", signature, ClassLoader.class);
			try {
				return (Class<?>) defineClassHandle.invoke(
						targetClassLoader,
						className,
						bytecode,
						0,
			...
			// Java 8 runtime, fall back to reflection.
		}
		Method reflectDefineClass = ClassLoader.class.getDeclaredMethod(
				"defineClass",
				String.class,
				byte[].class,
				int.class,
				int.class,
				ProtectionDomain.class);
		boolean accessible = reflectDefineClass.isAccessible();
		reflectDefineClass.setAccessible(true);
		try {
			return (Class<?>) reflectDefineClass.invoke(
					targetClassLoader,
					className,
					bytecode,
					0,
			...
			reflectDefineClass.setAccessible(accessible);
		}
	}

	private static void rethrowInvocationTargetException(InvocationTargetException e) throws Exception {
	...
	private static void rethrowThrowable(Throwable throwable) throws Exception {
